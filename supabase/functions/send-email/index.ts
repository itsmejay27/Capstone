 ...
 *     supabase secrets set RESEND_API_KEY=re_xxx RESEND_FROM="OMSC <no-reply@yourdomain>"
    ...
    <tr><td align="center">
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:560px;background:#ffffff;border:1px solid #e8e8ed;border-radius:14px;overflow:hidden;">
        <tr><td style="padding:18px 24px;border-bottom:1px solid #e8e8ed;">
          <span style="font-size:13px;font-weight:700;color:#4f46e5;letter-spacing:.02em;">OCCIDENTAL MINDORO STATE COLLEGE</span>
        ...
        <tr><td style="padding:26px 24px;">
          ...
          ${ctaUrl ? `<a href="${esc(ctaUrl)}" style="display:inline-block;margin-top:8px;background:#4f46e5;color:#fff;text-decoration:none;font-weight:700;font-size:14px;padding:11px 20px;border-radius:10px;">${esc(ctaLabel || 'Open in OMSC')}</a>` : ''}
        </td></tr>
        <tr><td style="padding:16px 24px;background:#fafafb;border-top:1px solid #e8e8ed;font-size:11px;color:#8e8e9e;">
          Sent by the OMSC AI Exam Generator. You are receiving this because you are enrolled in this class.
        </td></tr>
      </table>
    </td></tr>
  </table>
...
Deno.serve(async (req) => {
  ...
  const from = Deno.env.get('RESEND_FROM') || 'OMSC <onboarding@resend.dev>';
